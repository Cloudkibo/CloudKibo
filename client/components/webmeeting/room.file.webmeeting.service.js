...
angular.module('cloudKiboApp')
  .factory('MeetingRoomFileHangout', function MeetingRoomFileHangout($rootScope, MeetingRoomData, MeetingRoom, FileUtility, $log,$timeout) {
...
//      write_to_file(id, message, recieved_meta[message.fileid].chunks_recieved, hash);//id, rtc.usernames[id], message, recieved_meta[id].chunks_recieved, hash
    ...
    function download_file(id) {
      ...
      requesterid = MeetingRoomData.getcurrentid();
    ...
    function sendchunk(id, chunk_num, other_browser, rand, hash,requesterid) {
      ...
      reader.onload = function (event) {
        if (reader.readyState == FileReader.DONE) {
          ...
          MeetingRoomData.sendDataChannelMessageToUser(event.target.result,requesterid);
