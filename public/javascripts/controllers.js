function DashBoardController ($scope, EducationView){
	EducationView.isOn = true;
}

function ViewOtherProfileController ($scope, EducationView){
	EducationView.isOn = true;
}

function EditCourseController ($scope, EducationView, $timeout, $filter, $http, $window){
	EducationView.isOn = true;
	
	$scope.editFormInitTimeOut = function(){
	...
	$timeout($scope.editFormInitTimeOut, 1000);
	
	$scope.editCourse = function(){
		$http.post('/editcourse', JSON.stringify($scope.course))
...
function CreateCourseController ($scope, EducationView, $http, $window){
	EducationView.isOn = true;
	
	$scope.createCourseAlert = [];
	
	$scope.courseForm = {};
	$scope.showForm = true;
	
	$scope.doIshowForm = function(){
		return $scope.showForm;
	}
	
	$scope.addcreateCourseAlert = function(newtype, newMsg) {
		//console.log('Error', newtype, newMsg)
		$scope.createCourseAlert.push({type: newtype, msg: newMsg});
	};

	$scope.closecreateCourseAlert = function(index) {
		$scope.createCourseAlert.splice(index, 1);
	};
	
	$scope.createCourse = function(){
		$http.post('/createcourse', JSON.stringify($scope.courseForm))
		.success(function(data) {
			//console.log(data)
			$scope.showForm = false;
			$scope.addcreateCourseAlert(data.status, data.msg);
			//$window.location.href = '#mycourses';
		})
	}
}

function DisplayCourseController ($scope, EducationView, $http, $window, $upload, $timeout){
	EducationView.isOn = true;	
	
	$scope.httpMethod = 'POST';
	$scope.uploadTrials= 0;
	
	$scope.toggleRegistration = function(){
		$http.get('/toggleregistration/'+ $scope.course._id)
		.success(function(data) {
			console.log(data)
			$scope.course = data.msg;
		})
	}
	
	$scope.isRegistrationOpen = function(){
		return ($scope.course.registrationOpen == 'Yes') ? true : false ;
	}
	
	$scope.makeAnnouncement = function(){
		$http.post('/announce/'+ $scope.course._id, JSON.stringify($scope.newannouncement))
		.success(function(data) {
			console.log(data)
			$scope.newannouncement = {};
			$scope.announcements = data.msg;
		})
	}
...
function MyCoursesController ($scope, EducationView, $http, $window){
	EducationView.isOn = true;
	
	$http.get('/getmycourses')
	.success(function(data) {
		console.log(data)
		//$window.location.href = '#mycourses';
		$scope.courses = data.msg;
	})
	
	$scope.removeCourse = function(courseid){
		if(confirm('Are you sure you want to remove this course? All the students will be dropped from this course.')){
			$http.get('/removecourse/'+ courseid)
			.success(function(data) {
				//console.log(data)
				//$window.location.href = '#mycourses';
				$scope.courses = data.msg;
...
function MyFilesController ($scope, EducationView, $http, $timeout, $upload){
	EducationView.isOn = true;
	...
	$scope.start = function(index) {
		...
		{
			$scope.message = 'File is too big to be uploaded';
			$scope.progress[index] = -1;
		}
	}
}
...
function SearchCoursesController ($scope, EducationView){
	EducationView.isOn = true;
}

function HomeController ($scope, $http, socket, pc_config, pc_constraints, sdpConstraints, $timeout, EducationView){
	
	////////////////////////////////////////////////////////////////////////////////////////
	// Education User Interface Logic                                                     //
	///////////////////////////////////////////////////////////////////////////////////////
	
	$scope.openEducationView = function() {
		EducationView.isOn = !EducationView.isOn;
		//$scope.educationView = !$scope.educationView;
	}
	
	$scope.isEducationViewOn = function() {
		return EducationView.isOn;
		//return $scope.educationView;
	}
	
	$scope.joinAsStudent = function() {
		$http.post('/studentForm', JSON.stringify($scope.user))
		.success(function(data) {
			//console.log(data)
			$scope.user = data.msg;
		})
	...
	$scope.joinAsTeacher = function() {
		$http.post('/teacherForm', JSON.stringify($scope.user))
		.success(function(data) {
			//console.log(data)
			$scope.user = data.msg;
		})
	...
	$scope.updateProfile = function() {
		...
		if($scope.user.isStudent == 'Yes'){
			console.log($scope.student)
			$http.post('/editStudentProfile', JSON.stringify($scope.student))
			.success(function(data) {
				$scope.student = data;
			})
			.error(function(data) {
		...
		if($scope.user.isTeacher == 'Yes'){
			console.log($scope.student)
			$http.post('/editTeacherProfile', JSON.stringify($scope.teacher))
			.success(function(data) {
				$scope.teacher = data;
			})
			.error(function(data) {
...
function MeetingController ($scope, $http, socket, pc_config, pc_constraints, sdpConstraints, $timeout){
	...
	$scope.meetingRemoteVideoWidth = 560;
	$scope.firstVideoAdded = false;
	$scope.secondVideoAdded = false;
	$scope.thirdVideoAdded = false;
	$scope.forthVideoAdded = false;
	...
	function handleRemoteStreamAdded(event) {
		  ...
		  if($scope.firstVideoAdded == false){
			  ...
			  $scope.meetingRemoteVideoWidth = 560/2;
