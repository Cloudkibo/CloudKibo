...
exports.searchAccountByPhone = function (req, res, next) {
  ...
  User.find({phone: {$in: req.body.phonenumbers}}, function (err, gotUsers) {
    ...
    logger.serverLog('info', "Sending response to client : " + JSON.stringify({
        available: available,
              ...
              User.findById(req.body._id, function (err, gotUser) {
                ...
                gotUser.save(function (err2) {
                  if (err2) return res.json({
                    status: 'danger',
                    msg: 'Some error occurred on CloudKibo. Please contact us.'
                  });
  ...
  User.findOne({email: req.body.email}, function (err, gotUser) {
    ...
    configuration.findOne({}, function (err, gotConfig) {
      ...
      sendgrid.send(email, function (err, json) {
        ...
        res.send({
          status: 'success',
          msg: 'Password Reset Link has been sent to your email address. Check your spam or junk folder if you have not received our email.'
        });
  ...
  User.findOne({email: req.body.email}, function (err, gotUser) {
    ...
    configuration.findOne({}, function (err, gotConfig) {
      ...
      sendgrid.send(email, function (err, json) {
        ...
        res.send({
          status: 'success',
          msg: 'Cloudkibo username has been sent to your email address. Check your spam or junk folder if you have not received our email.'
        });
